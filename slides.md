...
# 開発者が直面する「設計に関する情報過多」
このPostを見てあるある〜と思ったのが、本セッションをしようと思い立ったきっかけになっています
...
- "このプロジェクトはClean Architectureで..."
- "ここはServiceLayerパターンを使って..."
- "SOLIDの原則に従って設計しましょう"
...
* 概念の適用範囲や限界を理解し、状況に応じて適切な選択を行うことができる
...
<div class="speech-bubble">
中程度の抽象度でモジュールやコンポーネントレベルの設計パターン
</div>
...
# アーキテクチャスタイル
Layered, Event-Driven, Microservices等
...
抽象度の上昇に伴い、影響範囲が広がり、変更コストが増加する為、決定の重要性が増す
...
# アーキテクチャパターンの役割
複雑化するエンタープライズアプリケーションの設計と統合に対する共通の課題に対処するために体系化
...
5. 分散システムの課題  
分散システムにおける一貫性、可用性、パーティション耐性（CAP定理）などの課題に対処する必要
6. 技術の進化への対応  
...
<Transform :scale="0.8" :origin="middle">
...
- 1995: SOLID原則（Robert C. Martin）
- 1999: DRY, YAGNI等の原則

【デザインパターンの体系化】
- 1987: Kent Beck & Ward Cunninghamがパターン言語を提唱
- 1994: GoF本「Design Patterns」出版
- オブジェクト指向設計の知見を集約
...
各PCにインストールして利用するパッケージを、バージョン管理とリリース計画に基づくチーム開発で進める時代
...
# 2010年代以降：クラウドネイティブ時代
クラウド上のサービス群を組み合わせて利用するシステムを、DevOpsによる自動化と分散開発で構築する時代
...
- MVCパターンはフルスタックフレームワークなら大体ある
- LaravelならActiveRecordパターン（Eloquent）
- SymfonyならData Mapperパターン（Doctrine）
- 必要に応じて取捨選択  
フレームワークを薄く使う
...
- 各レイヤーには構造とアーキテクチャがあり、SOLID原則が適用可能  
